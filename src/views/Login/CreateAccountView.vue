<template>
    ...
    <PopupSucces v-if="emailVerificationSent" :btn-msg="popupSuccessValues.btnMsg" :msg1="popupSuccessValues.msg1"
        :msg2="popupSuccessValues.msg2" />
    <div class="mt-[12%] w-full flex flex-col justify-center items-center md:mt-[2%] ">
        <div
            class="flex flex-col items-center justify-center w-full gap-1 font-poppins rounded-xl animate-fade md:w-9/12">
            <h1 class="mb-5 font-medium text-2xl text-[#006EAD] md:text-3xl">Create Account</h1>
            <form @submit.prevent
                class="flex flex-col items-center justify-center w-11/12 gap-1 p-1 font-poppins rounded-xl"
                autocomplete="off">
                <label class="flex items-center self-start gap-1 text-lg font-medium text-sky-800"
                    for="name">Name</label>
                <input v-model="name"
                    class="border w-full text-lg text-center h-12 border-[#006EAD] rounded-md focus:outline-none focus:border-[#006EAD] placeholder:text-slate-700 mb-5"
                    type="text" name="name" placeholder="Enter your name" required
                    title="Enter the name you want to have on your account">
                <label class="flex items-center self-start gap-1 text-lg font-medium text-sky-800"
                    for="email">Email</label>
                <input v-model="email"
                    class="border w-full text-lg text-center h-12 border-[#006EAD] rounded-md focus:outline-none focus:border-[#006EAD] placeholder:text-slate-700 mb-5"
                    type="email" name="email" placeholder="Email for your account" required
                    title="Enter the email address registered with your account">
                <label class="flex items-center self-start gap-1 text-lg font-medium text-sky-800"
                    for="password">Password</label>
                <input v-model="password"
                    class="border w-full text-lg text-center h-12 border-[#006EAD] rounded-md focus:outline-none focus:border-[#006EAD] placeholder:text-slate-700 mb-5"
                    type="password" name="password" placeholder="Set a password ">
                <label class="flex items-center self-start gap-1 text-lg font-medium text-sky-800"
                    for="confirm-password">Confirm Password</label>
                <input v-model="confirmPassword"
                    class="border w-full text-lg text-center h-12 border-[#006EAD] rounded-md focus:outline-none focus:border-[#006EAD] placeholder:text-slate-700 mb-5"
                    type="password" name="confirm-password" placeholder="Confirm your password">
                <ErrorAlert :message-error="msgError" :severity-error="severityError"
                    :error-user-data="incorrectUserData" />
                <div class="flex flex-wrap items-center justify-between w-full ">
                    <button @click="handleCreateAccount" type="submit"
                        class="w-36 p-2 text-white bg-[#006EAD] rounded-md shadow-md hover:bg-[#296480] ">Create
                        Account</button>
                    <div class="mt-3 animate-fade-left">
                        <RouterLink :to="{ name: 'login' }"
                            class="text-base font-medium underline text-slate-600 font-poppins">
...
import { addDoc, collection, getCountFromServer, getDoc, getDocs, getFirestore, query, Timestamp, where } from 'firebase/firestore';
...
const getUserContactCode = (): number => Math.floor(100000 + Math.random() * 900000); // function to get the a random contact code
...
const handleCreateAccount = async (): Promise<void> => { // Handle account creation and email verification
    if (!email.value || !password.value || !confirmPassword.value || !name.value) { // Check if all fields are filled
        ...
        try {// Create account and send email verification to user email
            // -Verify if user is trying to create an account that already exist (name)
            const q = query(userCollections, where('name', '==', name.value.trim()));
            const querySnapshot = await getDocs(q);
            if (!querySnapshot.empty) {
                incorrectUserData.value = true;
                msgError.value = 'Name already in use, go to login page and login with that email';
                severityError.value = 'high';
                waitingForVerification.value = false;
                return;
            }
            contactCode.value = getUserContactCode()
            const q_getContactCode = query(userCollections, where('contactCode', '==', contactCode.value))
            const getContactCode = (await getDocs(q_getContactCode)).docs[0];
            if (getContactCode && getContactCode.exists()) {   
                contactCode.value = getUserContactCode();
                handleCreateAccount();
                return;
            } 
